...
def create_tournee(tournee: TourneeCreate, db: Session = Depends(get_db)):
    ...
    for ordre, visite_id in enumerate(tournee.visites):
        ...
        visite = db.query(Visite).filter(Visite.id == visite_id).first()
        if not visite:
            raise HTTPException(status_code=404, detail=f"Visite {visite_id} introuvable")
...
def optimiser_tournee(tournee_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for v in visites_payload:
            route.append((v.latitude, v.longitude))

        route.append((cabinet.latitude, cabinet.longitude))
